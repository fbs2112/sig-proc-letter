    ...
    for index = 1:maxIt
        ...
        for k = (adapFiltLength(NIndex) + max(delayVector2)):globalLength
            ...
            if absoluteValueError > barGammaLin
                ...
                w(1:N(5),k+1) = w(1:N(5),k) + muLin(k)*G(1:N(5),1:N(5),k)*xAP(1:N(5),1)*((xAP(:,1)'*G(:,:,k)*xAP(:,1)+gamma*eye(L+1))\eye(L+1))*conj(e(k))*u;
                countLin(k,index) = 1;
            else
            ...
            if absoluteValueError > barGammaNonLin
                muNonLin(k) = 1 - barGammaNonLin/absoluteValueError;
                ...
                w(N(5)+1:end,k+1) = w(N(5)+1:end,k) + muNonLin(k)*G(N(5)+1:end,N(5)+1:end,k)*...
                    xAP(N(5)+1:end,1)*((xAP(:,1)'*G(:,:,k)*xAP(:,1)+gamma*eye(L+1))\eye(L+1))*conj(e(k))*u;
