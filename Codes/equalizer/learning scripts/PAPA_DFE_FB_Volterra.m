...
for FFIndex = 1:length(feedforwardLength)
    ...
    for FBIndex = 1:length(feedbackLength)
        ...
        delayVector = 1:feedforwardLength(FFIndex)+2;%adapFiltLength + 10;
        ...
        %         n = randn(globalLength,1) + randn(globalLength,1)*1i;
                ...
                for k = (adapFiltLength(FFIndex,FBIndex) + delayVector(delay)):globalLength
                   ...
                   if volterraFFFlag
                    ...
                    G(:,:,k) = diag(((1 - kappa*mu)/adapFiltLength(FFIndex,FBIndex)) + (kappa*mu*abs(w(:,k))/norm(w(:,k),1)));
...
save(['.' filesep 'results' filesep 'results28.mat'],'w4','e4');
